(function startDesktopApp() {
  ...
  function renderHome() {
    ...
    return `<section class="view">
      ${pageHead('GUAN XIANG · DESKTOP', '观象录', '不问远处的定数，只看当下有哪些条件可以改变。桌面版与微信小程序共用同一套离线内容和分析规则。')}
  ...
  function renderHistory() {
    const records = getRecords()
    return `<section class="view">
      ${pageHead('我的记录', '在事实变化后回看当时的判断', '记录只保存在当前 Windows 用户的浏览器本地存储中。')}
  ...
  function renderSettings() {
    ...
    return `<section class="view">
      ${pageHead('本地设置', '管理桌面版体验与数据', '设置、收藏和记录仅保存在本机；清理浏览器站点数据会删除这些内容。')}
      <div class="panel form-card">
        ...
        <div class="setting-row"><span><strong>隐私保护说明</strong><small>查看桌面版和小程序共同遵循的完整本地文本</small></span><button class="ghost" data-legal="privacy">查看全文</button></div>
        ...
        <div class="setting-row"><span><strong>清除全部本地数据</strong><small>删除桌面版记录、收藏和设置，不影响微信小程序数据</small></span><button class="danger" data-action="clear-all">清除数据</button></div>
      </div>
  ...
  document.addEventListener('click', (event) => {
    ...
    if (action === 'clear-history') {
      if (window.confirm('确认清空全部桌面版观象记录？此操作无法恢复。')) {
    ...
    if (action === 'clear-all') {
      if (window.confirm('确认清除桌面版全部记录、收藏和设置？此操作无法恢复。')) {
  ...
  if (!core) {
    app.innerHTML = '<section class="view"><div class="panel empty"><strong>桌面核心没有加载</strong>请重新运行项目生成命令。</div></section>'
    return
  }
  render()
})()
